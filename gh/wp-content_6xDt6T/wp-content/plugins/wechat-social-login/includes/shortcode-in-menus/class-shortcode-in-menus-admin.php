...
require_once 'class-shortcode-in-menus.php';
...
if ( ! class_exists( 'Shortcode_In_Menus_Admin' ) && class_exists( 'Shortcode_In_Menus' ) ) {
	...
	class Shortcode_In_Menus_Admin extends Shortcode_In_Menus {
		...
		public function setup_meta_box() {
			add_meta_box( 'add-shortcode-section', '登录按钮', array( $this, 'meta_box' ), 'nav-menus', 'side', 'default' );
		}
		...
		public function enqueue( $hook ) {
			...
			wp_enqueue_script( 'gs-sim-admin', XH_SOCIAL_URL . '/assets/js/shortcode-in-menus.js', array( 'nav-menu' ), 1.0, true );
		}
		...
		public function meta_box() {
			...
			<div class="gs-sim-div" id="gs-sim-div">
				...
				<p id="menu-item-title-wrap">
					<label for="gs-sim-title">标题</label>
					<input id="gs-sim-title" name="menu-item[<?php echo esc_attr( $nav_menu_placeholder ); ?>][menu-item-title]" value="登录" type="text" class="regular-text menu-item-textbox" title="标题" style="width:100%" />
				</p>
				<p id="menu-item-html-wrap">
                    <label for="gs-sim-title">文本/HTML/短码</label>
					<textarea style="width:100%;" rows="9" id="gs-sim-html" name="menu-item[<?php echo esc_attr( $nav_menu_placeholder ); ?>][menu-item-description]" class="code menu-item-textbox" title="文本/HTML/短码">[xh_social_login_short]</textarea>
				</p>
				<p class="button-controls">
					<span class="add-to-menu">
						<input type="submit" <?php wp_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="添加到菜单" name="add-gs-sim-menu-item" id="submit-gs-sim" />
            ...
            <script type="text/javascript">
                ;(function($){
                    if($("#add-shortcode-section").length){
                        $("#add-shortcode-section").css("display","block");
                    }
                })(jQuery);
            </script>
			<?php
