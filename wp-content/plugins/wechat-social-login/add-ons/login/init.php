    ...
    public function do_ajax(){
        ...
        switch($datas['tab']){
            ...
            case 'resetpassword':
                ...
                break;
            case 'email_register_vcode':
                $userdata = array();
                $fields=null;
                try {
                    $fields = $this->register_email_valication_fields();
                } catch (Exception $e) {
                ...
                $userdata =apply_filters('xh_social_findpassword_email_vcode_validate', $userdata);
                if(!XH_Social_Error::is_valid($userdata)){
                    echo $userdata->to_json();
                    exit;
                }

                if(!isset($userdata['user_email'])||!is_email($userdata['user_email'])){
                    echo XH_Social_Error::error_custom(__('Email is invalid!',XH_SOCIAL))->to_json();
                    exit;
                }

                $user = get_user_by('email', $userdata['user_email']);
                if($user){
                    XH_Social_Error::error_custom(__( 'Sorry, that email address is already used!' ))->output();
                    exit;
                }
                ...
                try {
                    if(!@wp_mail($userdata['user_email'], $subject, $message)){
                ...
                exit;
            case 'findpassword_email_vcode':
                $userdata = array();
                $fields=null;
                try {
                    $fields = $this->find_password_email_valication_fields();
                } catch (Exception $e) {
                    echo XH_Social_Error::error_custom($e->getMessage())->to_json();
                    exit;
                }
...
//         $wp_user = wp_authenticate($userdata['user_login'],  $userdata['user_pass']);
        ...
        //changed by hoter@xunhuweb.com 2019年1月24日 10:34:58
        /**
         ...
         */
        $user = apply_filters( 'authenticate', null, $userdata['user_login'],  $userdata['user_pass'] );
    ...
    private function register($datas){
        ...
        if(!isset($userdata['user_login'])){
            ...
            }else if(class_exists('Abstract_XH_Social_SMS_Api')&&isset($userdata[Abstract_XH_Social_SMS_Api::FIELD_MOBILE_NAME])){
                if(empty($userdata[Abstract_XH_Social_SMS_Api::FIELD_MOBILE_NAME])){
                    echo XH_Social_Error::error_custom(__('Mobile is required!',XH_SOCIAL))->to_json();
                    exit;
                }
                $userdata['user_login']=XH_Social::instance()->WP->generate_user_login($userdata[Abstract_XH_Social_SMS_Api::FIELD_MOBILE_NAME]);
            }
        }

        if(!isset($userdata['user_login'])|| empty($userdata['user_login'])){
            $userdata['user_login'] = XH_Social::instance()->WP->generate_user_login(time());
        }
        ...
        if(!get_option('users_can_register')){
            echo XH_Social_Error::error_custom('WordPress没开启注册-常规设置-开启允许任何人注册！')->to_json();
            exit;
        }
        ...
        $this->clear_email_validate_code();
    ...
    public function disable_wp_login(){
        ...
        if('yes'!=$this->get_option('disable_wp')){
            $cover=false;
        }else if($action=='login'&&isset($_REQUEST['r'])&&$_REQUEST['r']=='admin'){
            $cover=false;
        }
...
//             }
        ...
        if(!apply_filters('xh_social_disable_wp_login',$cover)){
            return;
        }
    ...
    public function page_login_login_fields(){
        ...
        $fields['login_name']=array(
                ...
                'validate'=>function($name,$datas,$settings){
                    $user_login =isset($_POST[$name])?(trim($_POST[$name])):'';
...
//         );
        ...
        //显示验证码
        if('yes'!=$this->get_option('register_email')&&'yes'==$this->get_option('register_with_captcha')){
            $captcha_fields = XH_Social::instance()->WP->get_captcha_fields();
            $fields = apply_filters('xh_social_page_login_register_fields',array_merge($fields,$captcha_fields),5.1);
        }
    ...
    public function get_captcha_email_fields($social_key = 'social_captcha_email') {
        $fields['captcha_email'] = array(
            'social_key' => $social_key,
            'type' => array($this,'create_captcha_form'),
            'validate'=>array($this,'validate_captcha')
        );
        ...
        <script type="text/javascript">
            (function($){
                ...
                $('#btn-code-<?php echo esc_attr($html_id);?>').click(function(){
                    ...
                    var url="<?php echo XH_Social::instance()->ajax_url()?>";
                    $.ajax({
                        url: url,
                        type: 'post',
                        timeout: 60 * 1000,
                        ...
                        success: function(m) {
                        	var callback = {
        						...
        						retry:function(){
        							$('#btn-code-<?php echo esc_attr($html_id);?>').click();
            					},
        						data:m
        		    		};
    ...
    public function validate_captcha($name, $datas, $settings){
        ...
        if(empty($code_post)){
            return XH_Social_Error::error_custom('邮箱验证码不能为空');
        }
    ...
    public function page_login_findpassword_methods(){
        ...
        switch ($findpassword_email_mode){
            ...
            case 'link':
                ...
                break;
            case 'code':
                $email_fields=$this->find_password_email_valication_fields(false);
    ...
    public function register_email_valication_fields(){
        $fields = apply_filters('xh_social_register_email_valication_fields',array(),0);
        ...
        $fields['register_user_email'] =  array(
            ...
            'validate'=>function ($name,$datas,$settings){
                ...
                if(empty($email)){
                    return XH_Social_Error::error_custom(__('Email is required!',XH_SOCIAL));
                }

                if(!is_email($email)){
                    return XH_Social_Error::error_custom(__('Invalid Email.',XH_SOCIAL));
                }

//                 $wp_user = get_user_by('email', $email);
//                 if($wp_user){
//                     return XH_Social_Error::error_custom(__( 'Sorry, that email address is already used!' ));
//                 }
                ...
                if(!empty($last_send_email)&&$last_send_email!= $datas['user_email']){
                    XH_Social::instance()->WP->clear_captcha();
                    $api = XH_Social_Add_On_Login::instance();
                    $api->clear_email_validate_code();
                }

                XH_Social::instance()->session->set('social_login_email_last_send',$datas['user_email']);
                return $datas;
            }
        );

        $fields=apply_filters('xh_social_register_email_valication_fields',$fields,1);

        if('yes'==$this->get_option('register_email')){

            if('yes'!=XH_Social_Email_Api::instance()->get_option('disabled_captcha')){
                $captcha_fields =XH_Social::instance()->WP->get_captcha_fields('register_captcha_email');

                $_fields['register_captcha_email'] = $captcha_fields['captcha'];
                $_fields['register_captcha_email']['section']=array('code');

                $fields=apply_filters('xh_social_register_email_valication_fields',array_merge($fields,$_fields),2);
            }

            $fields['register_email_vcode']= array(
                'validate'=>function($name,$datas,$settings){
                    $code_post =isset($_REQUEST[$name])?trim($_REQUEST[$name]):'';
                    if(empty($code_post)){
        ...
        return apply_filters('xh_social_register_email_valication_fields',$fields,3);
    }

    public function find_password_email_valication_fields(){
        $fields = apply_filters('xh_social_find_password_email_valication_fields',array(),0);
        ...
        $fields=apply_filters('xh_social_find_password_email_valication_fields',$fields,1);
        ...
        if('yes'!=XH_Social_Email_Api::instance()->get_option('disabled_captcha')){
            ...
            $fields=apply_filters('xh_social_find_password_email_valication_fields',array_merge($fields,$_fields),2);
        }
        ...
        return apply_filters('xh_social_find_password_email_valication_fields',$fields,3);
    }
