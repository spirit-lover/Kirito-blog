...
class XH_Social_Channel_Wechat extends Abstract_XH_Social_Settings_Channel{    
    ...
    public function update_wp_user_info($ext_user_id,$wp_user_id=null){
        ...
        try {
           XH_Social::instance()->WP->convert_remoteimage_to_local($wp_user_id, $ext_user_info['user_img']);
        } catch (Exception $e) {
    ...
    public function create_ext_user_info($prefix,$user_data,$wp_user_id,$uid){
        ...
        $wp_user_id = apply_filters('wsocial_pre_wechat_login',$wp_user_id, $update);
...
class XH_Social_Channel_Wechat_Model extends Abstract_XH_Social_Schema{
    ...
    public function init(){
        ...
        $wpdb->query("CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}xh_social_channel_wechat` (
                        ...
                        `op_openid` VARCHAR(128) NULL DEFAULT NULL,
                        `mp_openid` VARCHAR(128) NULL DEFAULT NULL,
                        `sp_openid` VARCHAR(128) NULL DEFAULT NULL,
                    ...
                    UNIQUE INDEX `sp_openid_unique` (`sp_openid`),
        ...
        try {
            ...
            if($wpdb->get_var (
                ...
                AND COLUMN_NAME = 'sp_openid'" ) != "{$wpdb->prefix}xh_social_channel_wechat"){
            
                $wpdb->query ( "ALTER TABLE `{$wpdb->prefix}xh_social_channel_wechat` ADD COLUMN `sp_openid` varchar(128) NULL DEFAULT NULL;" );
                if(!empty($wpdb->last_error)){
                    XH_Social_Log::error($wpdb->last_error);
                    throw new Exception($wpdb->last_error);
                }
            
                if($wpdb->get_var("show index from `{$wpdb->prefix}xh_social_channel_wechat` where Column_name='sp_openid' and Key_name='sp_openid_unique'")!= "{$wpdb->prefix}xh_social_channel_wechat"){
                    $wpdb->query ( "ALTER TABLE `{$wpdb->prefix}xh_social_channel_wechat` ADD UNIQUE INDEX sp_openid_unique(sp_openid);" );
                    if(!empty($wpdb->last_error)){
                        XH_Social_Log::error($wpdb->last_error);
                        throw new Exception($wpdb->last_error);
                    }
