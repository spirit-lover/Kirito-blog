...
abstract class Abstract_XH_Social_Settings_Channel extends Abstract_XH_Social_Settings{
    ...
    public function filter_display_name($nickname_or_loginname_or_displayname){
       return XH_Social::instance()->WP->filter_display_name($nickname_or_loginname_or_displayname);
    }
    ...
    public function wp_insert_user_Info($ext_user_id,$userdata){
        if(!get_option('users_can_register')){
            return XH_Social_Error::error_custom('WordPress没开启注册-常规设置-开启允许任何人注册！');
        }
        ...
        if(isset($session['ext_user_id'])&&$session['ext_user_id']==$ext_user_id){
            ...
            if($boundary>$now){
                return XH_Social_Error::error_custom(sprintf(__('Please try again after %s seconds!',XH_SOCIAL),($boundary-$now)));
            }
        }
        ...
        if(!function_exists('wsocial_ignore_wp_redirect')){
            function wsocial_ignore_wp_redirect($location, $status){
                return null;
            }
        }

        add_filter('wp_redirect', 'wsocial_ignore_wp_redirect',10,2);
        ...
        remove_filter('wp_redirect', 'wsocial_ignore_wp_redirect',10);
    ...
    public function bindinfo($wp_user_id){
        ...
        ob_start();
        $ext_user = $this->get_ext_user_info_by_wp($wp_user_id);
        if($ext_user){
            ?>
             <span class="xh-text" style="margin-right:8px;"><?php echo isset($ext_user['nickname'])?$ext_user['nickname']:'';?></span>
