...
class XH_Social_Add_On_Social_Wechat extends Abstract_XH_Social_Add_Ons{
    ...
    public function on_load(){
        ...
        add_filter('xunhuweb_per_wechat_login', __CLASS__.'::wrest_per_login',10,2);
    }
    
    public static function wrest_per_login($wp_user_id,$update){
        if($wp_user_id){
            return $wp_user_id;
        }
    
        if(defined('wsocial_do_pre_login')){
            return $wp_user_id;
        }
    
        define('wsocial_do_pre_login',1);
        ...
        if($addon&&version_compare($addon->version, '1.0.5','>=') &&$wechatapi&&$wechatapi instanceof XH_Social_Channel_Wechat){
            ...
            try{
                $ext_user_id = $wechatapi->create_ext_user_info('sp', $update, $wp_user_id, $uid);
                $wp_user = $wechatapi->update_wp_user_info($ext_user_id,$wp_user_id);
                if($wp_user instanceof XH_Social_Error){
                    return new WP_Error($wp_user->errcode,$wp_user->errmsg,array( 'status' => absint($wp_user->errcode)));
                }
                $wp_user_id = $wp_user->ID;
            }catch (Exception $e){
                WRest_Log::error("social 异常:".$e->getMessage());
                throw $e;
            }
        }
        return $wp_user_id;
