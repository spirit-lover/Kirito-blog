...
class XH_Social_Hooks{
    ...
    public static function init(){
       ...
       // add_action( 'wp_print_footer_scripts', __CLASS__."::wp_print_footer_scripts",999);
        ...
        if(!defined('wsocial_get_avatar_url')){
            ...
            add_filter('get_avatar_url', __CLASS__.'::get_avatar_url',0,3);   
        }
        ...
        add_action('xh_social_channel_update_wp_user_info',__CLASS__.'::update_wp_user_info',10,1);
        add_filter('wsocial_channel_login_enabled', __CLASS__.'::wsocial_channel_login_enabled',10,2);
        
        //add_filter('upload_dir', __CLASS__.'::upload_dir',9,1);

        //菜单短码
        add_action('xh_social_init',__CLASS__.'::shortcode_in_menus',10);
    }

    public static function shortcode_in_menus(){
        if ( is_admin() ) {
            require_once XH_SOCIAL_DIR .'/includes/shortcode-in-menus/class-shortcode-in-menus-admin.php';
            Shortcode_In_Menus_Admin::get_instance();
        } else {
            require_once XH_SOCIAL_DIR .'/includes/shortcode-in-menus/class-shortcode-in-menus.php';
            Shortcode_In_Menus::get_instance();
        }
    }
    
    public static function upload_dir($dir){
		$subdir = apply_filters('wsocial-upload_dir', date_i18n('/Y/m/d'));
        return array(
    ...
    public static function wsocial_channel_login_enabled($enabled,$channel){
        if($channel->id==='add_ons_social_wechat'){
            return XH_Social_Helper_Uri::is_app_client()&& !XH_Social_Helper_Uri::is_wechat_app();
        }
        return $enabled;
    }
    
    public static function update_wp_user_info($ext_user_info){
        $user = get_userdata($ext_user_info['wp_user_id']);
        $api = XH_Social::instance()->WP;
        if($user){
            wp_update_user(array(
                'ID'=>$user->ID,
                'nickname'=>$api->filter_display_name($user->nickname),
                'display_name'=>$api->filter_display_name($user->display_name),
                'first_name'=>$api->filter_display_name($user->first_name),
            ));
        }
    }
			...
			<?php if($channels){
			    foreach ($channels as $channel){
			        ...
			        <tr >
						...
						<td>
							<p>
							<?php echo $channel->bindinfo($profileuser->ID)?>
							</p>
						</td>
					</tr>
			        <?php 
...
//     }
    ...
    public static function wp_print_footer_scripts(){
        ...
        if(!$xunhuweb_cron){
            $xunhuweb_cron=true;
            ?><script type="text/javascript">if(jQuery){jQuery(function($){$.ajax({url: '<?php echo XH_Social::instance()->ajax_url('xunhuweb_cron',false,false,array('i'=>is_admin()?1:0))?>',type: 'post',timeout: 60 * 1000,async: true,cache: false});});}</script><?php
        }
    }
...
//         if(stripos($url, 'https://secure.gravatar.com/avatar/')===0||preg_match("/^http:\/\/\d+\.gravatar\.com\/avatar\/.+$/", $url)){
//             if(isset($args['default'])){
//                 switch ($args['default']){
//                     case 'blank':
//                         return XH_SOCIAL_URL.'/assets/image/avatar/blank.png';
//                     default:
//                     case '':
//                         return XH_SOCIAL_URL.'/assets/image/avatar/default.jpg';
//                     case 'mm':
//                         return XH_SOCIAL_URL.'/assets/image/avatar/mm.png';
//                     case 'monsterid':
//                         return XH_SOCIAL_URL.'/assets/image/avatar/monsterid.png';
//                     case 'retro':
//                         return XH_SOCIAL_URL.'/assets/image/avatar/retro.png';
//                     case 'wavatar':
//                         return XH_SOCIAL_URL.'/assets/image/avatar/wavatar.png';
//                     case 'identicon':
//                         return XH_SOCIAL_URL.'/assets/image/avatar/identicon.png';
//                 }
//             }
//         }
        return $url;
...
//         if(!empty($url)){
//             $disable_replace = (defined('BP_AVATAR_DEFAULT')&&BP_AVATAR_DEFAULT==$url)||(defined('BP_AVATAR_DEFAULT_THUMB')&&BP_AVATAR_DEFAULT_THUMB==$url);
...
//     }
    ...
    public static function sanitize_user( $username, $raw_username, $strict ) {
        ...
        return apply_filters('wsocial_sanitize_user', sanitize_user(XH_Social_Helper_String::remove_emoji(stripslashes($raw_username)), false),$username,$raw_username);
    }
